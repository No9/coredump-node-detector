...
kubectl create secret generic cos-write-access --type=ibm/ibmc-s3fs --from-literal=access-key=$ACCESS_KEY_ID --from-literal=secret-key=$SECRET_ACCESS_KEY -n kcdt
